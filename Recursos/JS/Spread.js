...
var Controller = function(container, size, position) {
  ...
  var onComplete = null;
  var completeCount = 0;
  //var tileColor = options.ColorInicial ? color() : options.color;
  var tileColor;

  if (options.ColorInicial){
  	options.ColorInicial = false;
  	tileColor = options.color;
  } else {
  	tileColor = color();
  }
...
setTimeout(start, 1000); // Inicio auto